...
class QLearning:
	...
	def __init__(self, env, alpha=0.5, gamma=0.95, epsilon=0.1, episodes=10):
		...
		self.q_table = np.zeros((self.env.size, self.env.size,
								 4))
...
for i in range(10):
	...
	def update(i):
		...
		# gen = (j for j, start in enumerate(starts) if start > i)  returns a generator if 0 to len (starts) - 1
		...
		if current_episode < 0:
			...
			steps = i - starts[current_episode] + 1  # reset steps to zero at the start of each episode

		ax.set_title(f"Episode: {current_episode + 1}, Total Reward: {cumulative_reward:.2f}, Steps: {steps}")
		grid = np.zeros((env.size, env.size))
		for obstacle in env.obstacles:
			grid[obstacle] = -1
		grid[env.goal] = 1
		grid[states[
			i]] = 0.5  # Use states[i] instead of env.state cuz env.state becomes the goal state at the end (4,4) if grid size 5x5
